import { A as AstroError, f as ActionNotFoundError } from './astro/assets-service_CI-QSHFT.mjs';

async function getAction(path) {
  const pathKeys = path.replace(/^.*\/_actions\//, "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_pBPeAztR.mjs');
